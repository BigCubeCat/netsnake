...
const (
	...
	NodeRole_VIEWER NodeRole = 3 // Наблюдатель, похож на NORMAL, но не имеет змеи в статусе ALIVE,
)
...
const (
	...
	PlayerType_ROBOT PlayerType = 1 // Робот, управляет своей змеёй с помощью алгоритма (это не нужно
)
...
const (
	...
	GameState_Snake_ZOMBIE GameState_Snake_SnakeState = 1 // Змея принадлежала игроку, который вышел из игры, она
)
...
type GamePlayer struct {
	...
	IpAddress *string                `protobuf:"bytes,3,opt,name=ip_address,json=ipAddress" json:"ip_address,omitempty"` // IPv4 или IPv6 адрес игрока в виде строки. Отсутствует в описании
	// игрока-отправителя сообщения
	Port *int32 `protobuf:"varint,4,opt,name=port" json:"port,omitempty"` // Порт UDP-сокета игрока. Отсутствует в описании
	// игрока-отправителя сообщения
...
type GameConfig struct {
	...
	StateDelayMs  *int32                 `protobuf:"varint,5,opt,name=state_delay_ms,json=stateDelayMs,def=1000" json:"state_delay_ms,omitempty"` // Задержка между ходами (сменой состояний) в игре, в
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type GameState struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	StateOrder *int32                 `protobuf:"varint,1,req,name=state_order,json=stateOrder" json:"state_order,omitempty"` // Порядковый номер состояния, уникален в
	// пределах игры, монотонно возрастает
...
type GameMessage struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	MsgSeq *int64                 `protobuf:"varint,1,req,name=msg_seq,json=msgSeq" json:"msg_seq,omitempty"` // Порядковый номер сообщения, уникален для отправителя в пределах
	// игры, монотонно возрастает
	SenderId *int32 `protobuf:"varint,10,opt,name=sender_id,json=senderId" json:"sender_id,omitempty"` // ID игрока-отправителя этого сообщения
	// (обязательно для AckMsg и RoleChangeMsg)
	ReceiverId *int32 `protobuf:"varint,11,opt,name=receiver_id,json=receiverId" json:"receiver_id,omitempty"` // ID игрока-получателя этого сообщения
	// (обязательно для AckMsg и RoleChangeMsg)
	// Тип сообщения
	//
...
type GameMessage_JoinMsg struct {
	...
	RequestedRole *NodeRole              `protobuf:"varint,5,req,name=requested_role,json=requestedRole,enum=snakes.NodeRole" json:"requested_role,omitempty"` // NORMAL, если хотим играть; VIEWER, если хотим только понаблюдать;
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type GameMessage_ErrorMsg struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ErrorMessage  *string                `protobuf:"bytes,1,req,name=error_message,json=errorMessage" json:"error_message,omitempty"` // Строковое сообщение, нужно отобразить его на экране, не блокируя
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
// 1. от заместителя другим игрокам о том, что пора начинать считать его
// главным (sender_role = MASTER)
// 2. от осознанно выходящего игрока (sender_role = VIEWER)
// 3. от главного к умершему игроку (receiver_role = VIEWER)
// 4. в комбинации с 1,2 или отдельно от них: назначение кого-то заместителем
// (receiver_role = DEPUTY)
// 5. в комбинации с 2 от главного узла заместителю о том, что он становится
// главным (receiver_role = MASTER)
...
var file_snakes_proto_rawDesc = []byte{
	...
	0x52, 0x49, 0x47, 0x48, 0x54, 0x10, 0x04, 0x42, 0x46, 0x0a, 0x17, 0x6d, 0x65, 0x2e, 0x69, 0x70,
	0x70, 0x6f, 0x6c, 0x69, 0x74, 0x6f, 0x76, 0x2e, 0x66, 0x69, 0x74, 0x2e, 0x73, 0x6e, 0x61, 0x6b,
	0x65, 0x73, 0x42, 0x0b, 0x53, 0x6e, 0x61, 0x6b, 0x65, 0x73, 0x50, 0x72, 0x6f, 0x74, 0x6f, 0x5a,
	0x1e, 0x67, 0x69, 0x74, 0x68, 0x75, 0x62, 0x2e, 0x63, 0x6f, 0x6d, 0x2f, 0x62, 0x69, 0x67, 0x63,
	0x75, 0x62, 0x65, 0x63, 0x61, 0x74, 0x2f, 0x6e, 0x65, 0x74, 0x73, 0x6e, 0x61, 0x6b, 0x65,
}
